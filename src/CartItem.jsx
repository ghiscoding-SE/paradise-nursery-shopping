...
const Cart = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () =>  cart.reduce((total, item) => total + item.quantity * parseFloat(item.cost.slice(1)), 0).toFixed(2);
  const calculateTotalCost = (item) => (item.quantity * parseFloat(item.cost.slice(1))).toFixed(2);
